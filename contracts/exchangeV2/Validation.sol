...
abstract contract Validation is IValidation, Signatures {
    ...
    function _validateListingFromBatch(
        ...
        uint256 amountTaken = amountTaken[order.trader][bytes32(order.salt)][
            listingIndex
        ];
        uint256 pendingAmountTaken = pendingAmountTaken[exchange.index][
            listingIndex
        ];
        ...
        unchecked {
            validListing =
                ...
                type(uint256).max - amountTaken &&
                amountTaken + pendingAmountTaken + takerAmount <=
                listing.amount;
        }
    }
